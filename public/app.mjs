...
function renderHome() {
  ...
  return `<section class="hero home-hero"><div class="hero-copy"><div class="hero-context"><span>${escapeHtml(f.name)}</span><span aria-hidden="true">·</span><strong>${escapeHtml(roles[state.role].name)}</strong></div><p class="eyebrow">ONE PRODUCT · TWO FIELDS</p><h1>${roleCopy[0]}</h1><p>${roleCopy[1]}</p><div class="hero-actions"><a class="button primary" href="#${roleCopy[3]}">${roleCopy[2]}</a><a class="button secondary" href="#career">신직무 경로 보기</a></div><p class="hero-role-summary">${escapeHtml(roles[state.role].summary)}</p></div><figure class="hero-visual"><img src="${syntheticAsset()}" alt="${syntheticAlt()}"><figcaption>AI-generated synthetic example · 실제 수집/승인 학습 데이터 아님</figcaption></figure></section>
...
function renderDataset() {
  ...
  return `${pageHead("RIGHTS-AWARE DATASET", "출처·권리·검수 상태를 한 버전에 묶습니다", "승인된 항목만 게시 준비 상태가 되며 접근 결정은 검수와 별도입니다.")}
    <div class="grid main-side"><section class="card dataset-overview"><div class="card-top"><div><p class="eyebrow">${field().taskId} · ${d.version}</p><h2>${field().task} 데이터셋</h2></div>${badge(datasetLabel(d.status), d.status === "published" ? "green" : "amber")}</div>
    <figure class="dataset-spotlight"><img src="${syntheticAsset()}" alt="${syntheticAlt()}"><figcaption><strong>${escapeHtml(field().data)}</strong><span>AI-generated synthetic example · 실제 수집 데이터 아님</span></figcaption></figure>
...
function renderCareer() {
  ...
  return `${pageHead("PROPOSED CAREER PATHS", "현장 AI 신직무를 증거 중심으로 보여 줍니다", "순위나 작업 건수가 아니라 교육·허용 범위·독립 검수·현장 맥락의 연결을 확인합니다.")}
    <section class="career-intro"><div><p class="eyebrow">HUMAN WORK AROUND PHYSICAL AI</p><h2>현장을 이해하고, 품질과 권리를 책임지는 여섯 가지 경로</h2></div><ol><li>교육과 허용 범위</li><li>수집과 독립 검수</li><li>권리와 재수집 계보</li></ol></section>
